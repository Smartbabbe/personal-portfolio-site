...
import VAFeaturedProjectCard from "./VAfeaturedprojectcard";
import VAMiniSampleTile from "./VAMiniSampleTile";
import VAModalPreview from "./VAModalPreview";
import type { PortfolioCategory, SampleProject } from "../data/vaPortfolioData";
...
export default function VAPortfolioSection({
  ...
  return (
    <div>
      ...
      <h3
        className={`text-2xl font-bold mb-8 flex items-center gap-3 transition-colors duration-300
          ${dark ? "text-white" : "text-gray-900"}`}
      >
        <div className={`w-1 h-8 rounded-full ${dark ? "bg-purple-500" : "bg-purple-600"}`} />
        {category.title}
      </h3>
