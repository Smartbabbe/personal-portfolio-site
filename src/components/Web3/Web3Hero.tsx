...
const EDGES = [[0,5],[1,5],[2,5],[3,5],[4,5],[0,2],[1,3],[3,4]]
...
export default function Web3Hero(_props: { onNavigate?: (p: string) => void }) {
  ...
  useEffect(() => {
    ...
    const draw = (t: number) => {
      ...
      EDGES.forEach(([a, b]) => {
        ...
        ctx.beginPath()
        ctx.arc(x1 + (x2-x1)*prog, y1 + (y2-y1)*prog, 2, 0, Math.PI*2)
      ...
      NODES.forEach((node, i) => {
        ...
        const isC = node.center, isH = hovered === i
        ...
        ctx.lineWidth = isC ? 1.5 : 1; ctx.stroke()
  ...
  return (
    <section className={`relative min-h-screen flex items-center pt-20 overflow-hidden ${d ? 'bg-zinc-950' : 'bg-stone-50'}`}>
      ...
      <div
        ...
        style={{
          top: '20%', left: '55%', width: '40vw', height: '40vw',
          borderRadius: '50%',
          background: d ? 'radial-gradient(circle, rgba(184,115,51,0.06), transparent 70%)' : 'radial-gradient(circle, rgba(184,115,51,0.08), transparent 70%)',
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-6 md:px-12 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center py-20">
        ...
        {/* LEFT */}
        <div>
          <div className={`inline-flex items-center gap-2 mb-10 transition-all duration-700 ${ready ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            <span className="relative w-2 h-2 rounded-full" style={{ background: d ? '#d4956a' : '#b87333' }}>
              <span className="absolute inset-0 rounded-full animate-ping" style={{ background: d ? '#d4956a' : '#b87333', opacity: 0.4 }} />
            </span>
            <span className={`text-[11px] font-semibold tracking-[0.2em] uppercase ${d ? 'text-amber-500/70' : 'text-amber-700/70'}`}>
          ...
          <p className={`text-base leading-relaxed mb-10 max-w-sm transition-all duration-700 delay-150 ${ready ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'} ${d ? 'text-zinc-400' : 'text-zinc-600'}`}>
            DeFi educator, community builder, on-chain analyst, and Web3 frontend developer. 5+ years in the space, building from Enugu.
          </p>

          <div className={`flex flex-wrap gap-2 mb-10 transition-all duration-700 delay-200 ${ready ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}`}>
            {['DeFi Educator','Community Builder','On-chain Analyst','Public Speaker','Web3 Frontend'].map(r => (
              <span key={r} className={`text-[11px] font-medium px-3 py-1.5 rounded-full border ${d ? 'border-zinc-700 text-zinc-400 bg-zinc-900' : 'border-stone-200 text-zinc-600 bg-white'}`}>{r}</span>
            ))}
          </div>

          <div className={`flex flex-wrap gap-3 mb-14 transition-all duration-700 delay-300 ${ready ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}`}>
            <button
              onClick={() => document.getElementById('w3-projects')?.scrollIntoView({ behavior: 'smooth' })}
          ...
          <div className={`flex gap-8 transition-all duration-700 delay-500 ${ready ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}`}>
            {[['5+','Years On-chain'],['2','Cohorts Taught'],['2','Public Talks'],['1','Chapter Founded']].map(([v,l]) => (
              <div key={l}>
                <p className="text-2xl font-black" style={{ background: 'linear-gradient(135deg, #b87333, #d4956a)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>{v}</p>
                <p className={`text-[11px] mt-0.5 ${d ? 'text-zinc-600' : 'text-zinc-500'}`}>{l}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className={`relative h-[420px] lg:h-[560px] transition-all duration-1000 delay-200 ${ready ? 'opacity-100' : 'opacity-0'}`}>
          <canvas
            ref={canvasRef}
          ...
          />
          {hovered !== null && (
            <div className={`absolute bottom-6 left-1/2 -translate-x-1/2 text-[11px] font-medium px-3 py-1.5 rounded-full border pointer-events-none ${d ? 'bg-zinc-900/90 border-zinc-700 text-amber-400' : 'bg-white/90 border-stone-200 text-amber-700'}`}>
              {NODES[hovered].label}
            </div>
          )}
          <p className={`absolute bottom-0 right-0 text-[10px] font-mono ${d ? 'text-zinc-700' : 'text-stone-400'}`}>hover the nodes</p>
        </div>
      </div>

      {/* Scroll cue */}
      <div className={`absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-1.5 ${d ? 'opacity-30' : 'opacity-40'}`}>
        <div className="w-px h-8" style={{ background: `linear-gradient(to bottom, transparent, #b87333)` }} />
