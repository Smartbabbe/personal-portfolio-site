import { X, FileText, Wrench, Lightbulb, Star } from "lucide-react";
import { useTheme } from "../contexts/ThemeContext";
import { useEffect } from "react";
import type { FeaturedProject, SampleProject } from "../data/vaPortfolioData";
...
export default function VAModalPreview({ project, onClose }: VAModalPreviewProps) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <FileText className={`w-14 h-14 ${dark ? "text-gray-600" : "text-gray-400"}`} />
          <p className={`text-xs text-center px-8 ${dark ? "text-gray-400" : "text-gray-500"}`}>
            {isFeatured(project) ? project.visuals[0]?.caption : project.imagePlaceholder}
          </p>
          <p className={`text-xs italic ${dark ? "text-gray-600" : "text-gray-400"}`}>
        ...
        <div className="p-6 space-y-6">
          ...
          {isFeatured(project) && (
            <>
              <div>
                <h4 className={`flex items-center gap-2 text-sm font-semibold uppercase tracking-wide mb-2
          ...
          {/* Tools Used */}
          <div>
            <h4 className={`flex items-center gap-2 text-sm font-semibold uppercase tracking-wide mb-3
              ${dark ? "text-purple-400" : "text-purple-600"}`}>
          ...
          >
            <strong>Confidentiality Note:</strong> All data and visuals shown have been
            anonymized or recreated using mock information to respect client confidentiality.
          </div>
        </div>
      </div>
    </div>
